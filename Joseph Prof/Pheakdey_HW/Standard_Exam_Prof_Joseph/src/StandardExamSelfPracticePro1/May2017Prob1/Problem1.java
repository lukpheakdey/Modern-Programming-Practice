package StandardExamSelfPracticePro1.May2017Prob1;
...
//				.filter(  e->list1.contains(e) && !list2.contains(e) || !list1.contains(e) && list2.contains(e)) 
//				.collect(Collectors.toList());
		
		return Stream.concat(list1.stream(), list2.stream())
				.filter(l -> list1.contains(l) && !list2.contains(l) || !list1.contains(l) && list2.contains(l))
				.collect(Collectors.toList());
...
//				.collect(Collectors.toList());
		return list.stream()
				.filter(l -> l.getCity().equals("Fairfield"))
				.map(l -> l.getName())
				.sorted()
				.collect(Collectors.toList());
	}
